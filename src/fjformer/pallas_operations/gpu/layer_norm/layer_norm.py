...
def layer_norm_forward_kernel(
	x_ref,
	weight_ref,
	bias_ref,  # Input arrays
	o_ref,
	mean_ref=None,
	rstd_ref=None,  # Output arrays
	*,
	eps: float,
	block_size: int,
):
	...
	def mean_body(i, acc_ref):
		...
		mask = col_idx < n_col
		a = pl.load(
			x_ref, (col_idx,), mask=mask, other=0.0, eviction_policy="evict_last"
		).astype(jnp.float32)
		acc_ref[:] += a

	mean = (
		for_loop(pl.cdiv(n_col, block_size), mean_body, jnp.zeros(block_size)).sum() / n_col
	)
	...
	var = (
		for_loop(pl.cdiv(n_col, block_size), var_body, jnp.zeros(block_size)).sum() / n_col
	)
	...
	def body(i, _):
		...
		x = pl.load(
			x_ref, (col_idx,), mask=mask, other=0.0, eviction_policy="evict_first"
		).astype(jnp.float32)
...
def layer_norm_forward(
	...
	kernel = functools.partial(layer_norm_forward_kernel, eps=eps, block_size=block_size)
	out_shape = [
...
def layer_norm_backward_kernel_dx(
	...
	*,
	...
	def mean_body(i, acc_ref):
		...
		dout = pl.load(
			do_ref, (col_idx,), mask=mask, other=0.0, eviction_policy="evict_last"
		).astype(jnp.float32)
		weight = pl.load(
			weight_ref, (col_idx,), mask=mask, other=0.0, eviction_policy="evict_last"
		).astype(jnp.float32)
...
def layer_norm_backward_kernel_dw_db(
	...
	*,
	...
	def body(i, acc_ref):
		...
		a = pl.load(x_ref, (row_idx[:, None], col_idx[None]), mask=mask, other=0.0).astype(
			jnp.float32
		)
		dout = pl.load(
			do_ref, (row_idx[:, None], col_idx[None]), mask=mask, other=0.0
		).astype(jnp.float32)
		mean = pl.load(mean_ref, (row_idx,), mask=row_mask, other=0.0).astype(jnp.float32)
		rstd = pl.load(rstd_ref, (row_idx,), mask=row_mask, other=0.0).astype(jnp.float32)
		a_hat = (a - mean[:, None]) * rstd[:, None]
		dw_acc_ref, db_acc_ref = acc_ref
		dw_acc_ref[:] += (dout * a_hat).sum(axis=0)
		db_acc_ref[:] += dout.sum(axis=0)

	dw_acc, db_acc = for_loop(
		pl.cdiv(m, block_m), body, (jnp.zeros(block_n), jnp.zeros(block_n))
	)
...
def layer_norm_backward(
	...
	*shape_prefix, n = x.shape
	...
	kernel = functools.partial(
		layer_norm_backward_kernel_dx, eps=eps, block_size=block_size
	)
	...
	if n > 10240:
		...
		num_warps = 8
	kernel = functools.partial(
		layer_norm_backward_kernel_dw_db, eps=eps, block_m=block_m, block_n=block_n
	)
	out_shape_dwbias = [
	...
	dw, dbias = method(
		reshaped_x, weight, bias, reshaped_do, reshaped_mean, reshaped_rstd
	)
	return dx, dw, dbias
