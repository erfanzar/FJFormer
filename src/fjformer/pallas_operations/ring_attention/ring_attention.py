...
def _ring_attention_fwd(q, k, v, attn_bias, segment_ids, axis_name, float32_logits, blockwise_kwargs):
    ...
    def scan_kv_block(carry, idx):
        ...
        numerator, denominator, max_score = _blockwise_attention_fwd(q, k, v, (numerator, denominator, prev_max_score),
                                                                     ...
                                                                     bias=attn_bias, segment_ids=segment_ids,
                                                                     **blockwise_kwargs)
...
def _ring_attention_bwd(axis_name, float32_logits, blockwise_kwargs, res, g):
    ...
    def scan_kv_block(carry, idx):
        ...
        dq, dk, dv = _blockwise_attention_bwd(q, k, v, g, (dq, dk, dv, output, denominator, max_score),
                                              q_chunk_idx_start, k_chunk_idx_start, bias=attn_bias,
                                              segment_ids=segment_ids, **blockwise_kwargs)
...
def _blockwise_attention_fwd(q, k, v, carry, q_chunk_idx_start, k_chunk_idx_start, bias, segment_ids, causal,
                             query_chunk_size,
...
def _blockwise_attention_bwd(q, k, v, g, carry, q_chunk_idx_start, k_chunk_idx_start, bias, segment_ids, causal,
                             query_chunk_size, key_chunk_size, deterministic, dropout_rng, attn_pdrop, dtype, policy,
                             precision, prevent_cse):
    batch, q_len, num_heads, dim_per_head = q.shape
    batch, kv_len, num_heads, dim_per_head = k.shape
    batch, kv_len, num_heads, dim_per_head = v.shape
    num_q = q_len // query_chunk_size
