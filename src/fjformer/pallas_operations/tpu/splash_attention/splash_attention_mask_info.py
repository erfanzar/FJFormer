...
def _process_mask(
	...
	*,
	...
	block_id_to_block_coords: Dict[int, List[Tuple[int, ...]]] = collections.defaultdict(
		list
	)
	...
	unique_partial_mask_blocks = [
		pair[0] for pair in sorted(partial_mask_block_ids.items(), key=lambda x: x[1])
	]
	...
	all_head_shards_identical = all(
		head_shard_to_mask_ids[0] == x and len(x) == 1 for x in head_shard_to_mask_ids
	)
	...
	if shrink_grid and block_mask.shape[0] == head_shards and len(unique_masks) == 1:
		...
		for q_seq_len_shard in range(q_seq_shards):
			...
			assert (
				current_mask_next is None or current_block_mask.shape == current_mask_next.shape
			)
