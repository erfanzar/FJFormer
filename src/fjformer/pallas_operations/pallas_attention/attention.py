...
def flash_attention(
    ...
    if interpret is None:
        interpret = (
            not (seq_len / 16).is_integer()
            or jax.lib.xla_bridge.get_backend().platform == "cpu"
        )
    ...
    in_specs = [
        ...
        (
            ...
            if bias is None
                ...
                lambda _, j, k: (j, 0, 0, 0), (None, None, seq_len, seq_len)
            )
        ),
    ]
...
def _preprocess_backward_kernel(
    out_ref, d_out_ref, l_ref, new_d_out_ref, delta_ref, *, block_q: int
):
...
def _preprocess_backward(out, do, l, block_q: int, debug: bool, interpret: bool):
    ...
    do_scaled, delta = pl.pallas_call(
        ...
        debug=debug,
        interpret=interpret,
        name="flash_attention_preprocess_backward",
    )(out, do, l)
...
def _flash_attention_backward(
    ...
    if backward_pass_impl == "xla":
        ...
        if b is None:
            ...
            in_specs.insert(
                3,
                pl.BlockSpec(lambda j, k: (j, 0, 0, 0), (None, None, seq_len, seq_len)),
            )
