...
from ._errors import UninitializedAval

_dispatch = Dispatcher()
_primitive_ids = count()
warnings.filterwarnings(
	"ignore",
	message="Could not resolve the type hint of `~B`",
	module="plum.type",
)
warnings.filterwarnings(
	"ignore",
	message="Could not resolve the type hint of `~A`",
	module="plum.type",
)
...
def _implicit_outer(*in_vals):
	...
	# Here's the key:  We take the existing tracing context (trace_ctx.trace),
	# and use that to create the tracers.  This avoids the need for
	# new_base_main or a global "reset" operation.
	...
	out_vals = [trace.full_lower(t) for t in outs]  # Use full_lower
	yield out_vals


@lu.transformation
def _implicit_inner(*in_tracers):
...
class ImplicitArray(_ImplicitArrayBase):
	...
	def handle_primitive(self, primitive, *args, params):
		...
		flat_handler, out_tree = flatten_fun_nokwargs(handler, in_tree)
		outs = implicit_compact(flat_handler.call_wrapped)(*flat_args)
...
class ImplicitArrayTracer(core.Tracer):
	...
	def to_concrete_value(self):
		if core.is_concrete(self.value):
			return self.value.to_concrete_value()
		else:
			return None
...
def wrap_jaxpr(jaxpr, vals_with_implicits, return_closed=True):
	if isinstance(jaxpr, jax.core.ClosedJaxpr):
		literals = jaxpr.consts  # Changed from jaxpr.literals
		jaxpr = jaxpr.jaxpr
	else:
		literals = []
...
def _transform_jaxpr_output(jaxpr, jaxpr_args, orig_out_struct, out_transform):
	...
	flat_args, in_tree = jax.tree_util.tree_flatten(
		(jaxpr.consts, *jaxpr_args)
	)  # Changed from jaxpr.literals
	flat_fn, out_tree = flatten_fun_nokwargs(wrapped, in_tree)
	new_jaxpr, _, consts = pe.trace_to_jaxpr_dynamic(
		flat_fn, [core.get_aval(v) for v in flat_args]
	)
		...
		out_avals.append(
			branch_out_struct.unflatten(
				jax.eval_shape(
					...
					new_jaxpr.consts,  # Changed from jaxpr.literals
					*flat_inputs,
				)
			)
		)
...
def _handle_remat2(primitive, *vals, params):
	subfuns, bind_params = primitive.get_bind_params(params)
	new_jaxpr, flat_inputs, out_tree = wrap_jaxpr(
		params["jaxpr"], vals, return_closed=False
	)
	bind_params["jaxpr"] = new_jaxpr
	flat_consts = new_jaxpr.consts

	outs = primitive.bind(*subfuns, flat_consts, *flat_inputs, **bind_params)
	return jax.tree_util.tree_unflatten(out_tree, outs)
...
def _handle_scan(primitive, *vals, params):
	...
	for x in sliced_xs:
		if isinstance(x, ImplicitArray):
			assert len(x.shape) > 0, "Attempted to scan over a scalar."
	...
	flat_inputs = jax.tree_util.tree_leaves((jaxpr.consts, *consts, *carries, *xs))
